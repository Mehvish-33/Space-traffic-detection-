...
st.write("Provide the input values to predict the traffic density in space.")
...
if st.button("Predict Traffic Density"):
    ...
    # Visualization: Confidence Interval for Prediction
    st.subheader("Prediction Confidence Interval")
    prediction_range = (prediction[0] - 0.1, prediction[0] + 0.1)
    st.write(f"Estimated Traffic Density Range: {prediction_range[0]:.2f} - {prediction_range[1]:.2f}")

# Visualizations
st.sidebar.header("Visualizations")
if st.sidebar.checkbox("Show Traffic Density Distribution"):
    st.subheader("Traffic Density Distribution")
    plt.figure(figsize=(10, 6))
    sns.histplot(data['Traffic_Density'], kde=True, color='blue')
...
if st.sidebar.checkbox("Show Feature Importance"):
    st.subheader("Feature Importance (Correlation Heatmap)")
...
if st.sidebar.checkbox("Show Traffic Density Category Distribution"):
    st.subheader("Traffic Density Category Distribution")
    category_counts = data['Traffic_Density_Category'].value_counts().sort_index()
